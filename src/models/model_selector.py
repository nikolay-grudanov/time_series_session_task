...
class ModelSelector:
    ...
    """
    def __init__(self, timeout_seconds: int = 300):  # 5 minutes timeout
    ...
    def timeout_manager(self):
        ...
        if elapsed_time > self.timeout_seconds:
            raise TimeoutError(f"Model training exceeded {self.timeout_seconds} seconds")
    ...
    def train_all_models(self, data: pd.Series) -> dict[str, dict]:
        ...
        for model_name in ['LSTM', 'GRU', 'RNN']:
            try:
                with self.timeout_manager():
                    start_time = time.time()
                    result = self.neural_network_models.train_model(model_name, data)
                    elapsed_time = time.time() - start_time
                    ...
                    if 'model' in result and result['model'] is not None:
                        ...
                        logger.warning(f"Model {model_name} training failed: {result.get('error', 'Unknown error')}")
        ...
        try:
            with self.timeout_manager():
                start_time = time.time()
                stat_results = self.statistical_models.train_all_models(data)
                elapsed_time = time.time() - start_time
    ...
    def select_best_model(self) -> tuple[str, Any]:
        ...
        best_model_name = min(model_scores.keys(), key=lambda x: model_scores[x]['composite_score'])
        best_result = self.training_results[best_model_name]

        logger.info(f"Selected {best_model_name} as the best model with composite score: {model_scores[best_model_name]['composite_score']:.4f}")
        logger.info(f"Best model metrics - RMSE: {model_scores[best_model_name]['rmse']:.4f}, "
    ...
    def predict(self, forecast_days: int = 30) -> np.ndarray:
        ...
        if self.selected_model is None:
            raise ValueError("No model has been selected yet. Call select_best_model() first.")

        if self.selected_model_name in ['LSTM', 'GRU', 'RNN']:
            # Use neural network model for prediction
            # We need to pass the original training data to make predictions
            # This is a limitation - in practice, we'd need to store the last sequence
            raise NotImplementedError(
                "Neural network prediction requires the original training data. "
