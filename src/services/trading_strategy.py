...
class TradingStrategyService:
    ...
    def identify_optimal_trades(
        ...
        for i in range(max(short_window, long_window), len(forecast_series)):
            ...
            if (i >= short_window and i >= long_window and
                forecast_series_short_avg.iloc[i-1] <= forecast_series_long_avg.iloc[i-1] and
                forecast_series_short_avg.iloc[i] > forecast_series_long_avg.iloc[i]):
            ...
            elif (i >= short_window and i >= long_window and
                  forecast_series_short_avg.iloc[i-1] >= forecast_series_long_avg.iloc[i-1] and
                  forecast_series_short_avg.iloc[i] < forecast_series_long_avg.iloc[i]):
        ...
        for i in range(1, len(forecast_series)-1):
            ...
            if prev_price > curr_price < next_price:
                ...
                if not any(pd.Timestamp(sig['date']).date() == date.date() for sig in buy_signals):
                    buy_signals.append({
                        'date': date,
                        'price': curr_price,
            ...
            # Local maximum (potential sell point)
                ...
                if not any(pd.Timestamp(sig['date']).date() == date.date() for sig in sell_signals):
                    sell_signals.append({
                        'date': date,
                        'price': curr_price,
        ...
        if buy_signals and (not sell_signals or buy_signals[0]['date'] < sell_signals[0]['date']):
            # Start with buy
            last_signal_type = 'buy'
            filtered_buy_signals.append(buy_signals[0])

            for buy_sig in buy_signals[1:]:
                if last_signal_type == 'sell':
                    # Add this buy signal
                    filtered_buy_signals.append(buy_sig)
                    last_signal_type = 'buy'

            for sell_sig in sell_signals:
                if last_signal_type == 'buy':
                    # Add this sell signal
                    filtered_sell_signals.append(sell_sig)
                    last_signal_type = 'sell'
        elif sell_signals:
            # Start with sell
            last_signal_type = 'sell'
            filtered_sell_signals.append(sell_signals[0])

            for sell_sig in sell_signals[1:]:
                if last_signal_type == 'buy':
                    # Add this sell signal
                    filtered_sell_signals.append(sell_sig)
                    last_signal_type = 'sell'
        ...
        result = {
            ...
            'strategy_summary': self._generate_strategy_summary(top_buy_signals, top_sell_signals)
        }

        logger.info(f"Identified {len(top_buy_signals)} buy signals and {len(top_sell_signals)} sell signals")
        return result

    def _generate_strategy_summary(self, buy_signals: list[dict], sell_signals: list[dict]) -> str:
        """
    ...
    def generate_trading_recommendations(
        ...
        """
        logger.info(f"Generating trading recommendations with investment amount: ${investment_amount:,.2f}")

        # Identify optimal trades
        trade_signals = self.identify_optimal_trades(forecast_data, historical_data, forecast_dates)
        ...
        recommendations = {
            ...
            'recommendation_summary': self._generate_recommendation_summary(trade_signals, potential_profits)
        }
    ...
    def calculate_potential_profits(
        ...
        if shares_owned > 0:
            last_forecast_price = sell_signals[-1]['price']  # Use the last forecast price
        ...
        total_profit = sum(trade['profit'] for trade in trade_details if 'profit' in trade)

        # Calculate ROI
        roi_percentage = (total_profit / investment_amount) * 100 if investment_amount > 0 else 0
        ...
        return {
            ...
            'total_trades_executed': len([td for td in trade_details if td['action'] in ['BUY', 'SELL']])
        }

    def _generate_recommendation_summary(self, trade_signals: dict, potential_profits: dict) -> str:
        """
